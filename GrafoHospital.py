...
class GrafoHospitales:
    ...
    def agregar_hospital(
        ...
        if solicitar_datos:
            ...
            # Si no se solicitan datos, se verifica que los nombres no sean nulos
    ...
    def mejor_ruta_ambulancia(self):
        ...
        while not origen_valido:
            ...
            if origen.isdigit():  # Verificar que el ID sea un número entero
                ...
                if origen in self.hospitales:
                    ...
                    print(
                        f"El hospital con ID '{origen}' no existe. Por favor, ingrese un ID válido."
                    )
            else:
                print("El ID del hospital debe ser un número entero.")
        ...
        while not destino_valido:
            ...
            if destino.isdigit():  # Verificar que el ID sea un número entero
                ...
                if destino in self.hospitales:
                    ...
                    print(
                        f"El hospital con ID '{destino}' no existe. Por favor, ingrese un ID válido."
                    )
            else:
                print("El ID del hospital debe ser un número entero.")
        ...
        if ruta:
            ...
            print(
                " -> ".join(str(self.hospitales[hospital].nombre) for hospital in ruta)
            )  # Mostrar los nombres de los hospitales
            print(f"Distancia total: {distancia} km")
        else:
            print(
                f"No se encontró una ruta desde el hospital con ID {origen} a {destino}."
            )
    ...
    def hospitales_ejemplos(self):
        ...
        hospitales_ejemplo = [
            {"id": 1, "nombre": "Hospital A", "especialidad": "Cardiología"},
            {"id": 2, "nombre": "Hospital B", "especialidad": "Neurología"},
            {"id": 3, "nombre": "Hospital C", "especialidad": "Pediatría"},
            {"id": 4, "nombre": "Hospital D", "especialidad": "Traumatología"},
            {"id": 5, "nombre": "Hospital E", "especialidad": "Ginecología"},
        ]
        for hospital in hospitales_ejemplo:
            self.agregar_hospital(
                hospital["id"],
                hospital["nombre"],
    ...
    def conexiones_ejemplos(self):
        # Agregar conexiones entre hospitales
        conexiones_ejemplo = [
            {"hospital1": 1, "hospital2": 2, "distancia": 10},  # Hospital A <-> Hospital B
            {"hospital1": 2, "hospital2": 3, "distancia": 15},  # Hospital B <-> Hospital C
            {"hospital1": 3, "hospital2": 4, "distancia": 20},  # Hospital C <-> Hospital D
            {"hospital1": 4, "hospital2": 5, "distancia": 25},  # Hospital D <-> Hospital E
            {"hospital1": 1, "hospital2": 5, "distancia": 30},  # Hospital A <-> Hospital E
        ]
        
        for conexion in conexiones_ejemplo:
        ...
        print("Conexiones de ejemplo agregadas.\n")
