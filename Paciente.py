...
class Paciente:
    ...
    def detalle_completo(self):
        return (f"{self}\n"
                f"Historial: {', '.join(str(evento) for evento in self.historial)}\n"
                f"Medicamentos: {', '.join(str(medicamento) for medicamento in self.medicamentos)}\n"
                f"Enfermedades: {', '.join(str(enfermedad) for enfermedad in self.enfermedades)}")  # Incluir enfermedades