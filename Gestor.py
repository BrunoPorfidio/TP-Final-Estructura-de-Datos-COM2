...
class GestorPacientes:
    ...
    def agregar_paciente(self, nombre: str, edad: int, gravedad: int, id_hospital: int) -> None:
        ...
        if nodo is None:
            ...
            if paciente:
                ...
                if paciente in self.cola_prioridad.buscar_entrada:
                    ...
                    print(f"Paciente con ID {id_paciente} no encontrado en la cola de prioridad.")
            else:
                print(f"No se pudo obtener información del paciente con ID {id_paciente}.")
    ...
    def modificar_paciente(self) -> None:
        ...
        if continuar:
            ...
            nombre = input("Nuevo nombre (dejar en blanco si no se desea modificar): ").strip()
            if nombre == "":
                nombre = None  # Si no se proporciona nombre, se deja como None

            # Validar la entrada de edad con una bandera booleana
            edad_valida = False
            while not edad_valida:  # Ciclo que se repite hasta que se ingrese un valor válido o se deje vacío
                edad_input = input("Nueva edad (dejar en blanco si no se desea modificar): ").strip()
            ...
            while not gravedad_valida:  # Ciclo que se repite hasta que se ingrese un valor válido o se deje vacío
                gravedad_input = input("Nueva gravedad (dejar en blanco si no se desea modificar): ").strip()
        ...
        if continuar:
            ...
            if nodo:
                ...
                if gravedad is not None:  # Si se proporciona una gravedad válida
                    ...
                    self.cola_prioridad.actualizar_gravedad(paciente, gravedad)  # Actualizar gravedad en la cola de prioridad
    ...
    def agregar_evento_medico(self) -> None:
        ...
        while not tipo_es_valido:
            tipo = input("Tipo de evento (CONSULTA/DIAGNOSTICO/TRATAMIENTO): ").upper()  # Convertimos a mayúsculas para evitar errores de capitalización
            if tipo in ["CONSULTA", "DIAGNOSTICO", "TRATAMIENTO"]:
                tipo_es_valido = True
            else:
                print("Tipo de evento no válido. Debe ser CONSULTA, DIAGNOSTICO o TRATAMIENTO.")

        # Validación de los detalles del evento
        while not detalles_valido:
            detalles = input("Detalles del evento: ").strip()  # Eliminamos espacios innecesarios al inicio o final
    ...
    def agregar_medicamento(self) -> None:
        ...
        while not nombre_medicamento_valido:
            nombre_medicamento = input("Nombre del medicamento: ").strip()  # Eliminamos espacios en blanco al inicio y final
            if nombre_medicamento:  # Verificamos que el nombre del medicamento no esté vacío
        ...
        if paciente:
            ...
            print(f"Medicamento '{nombre_medicamento}' agregado al paciente con ID {id_paciente}.")
        else:
            print(f"No se encontró paciente con ID {id_paciente}.")
    ...
    def agregar_enfermedad(self) -> None:
        ...
        while not nombre_enfermedad_valido:
            nombre_enfermedad = input("Nombre de la enfermedad: ").strip()  # Eliminamos espacios en blanco al inicio y final
            if nombre_enfermedad:  # Verificamos que el nombre de la enfermedad no esté vacío
        ...
        if paciente:
            ...
            print(f"Enfermedad '{nombre_enfermedad}' agregada al paciente con ID {id_paciente}.")
        else:
            print(f"No se encontró paciente con ID {id_paciente}.")
    ...
    def atender_paciente_mas_grave(self) -> None:
        paciente_atendido = None  # Inicializamos el valor de retorno por defecto como None

        # Validación para asegurarse de que la cola no esté vacía
        if not self.cola_prioridad.esta_vacia():
            paciente = self.cola_prioridad.desacolar()  # Desacolar al paciente más grave
            
            # Validación: Verificamos que el paciente no sea None
            if paciente:
                self.arbol_pacientes.eliminar(paciente.id)  # Eliminamos al paciente del árbol
    ...
    def validar_id(self) -> int:
        ...
        while not id_es_valido:
            ...
            if id_paciente_input.isdigit():
                ...
                print("El ID ingresado no es válido. Debe ser un número entero positivo.")
        ...
        while not id_es_valido:
            ...
            if entrada.isdigit():
                ...
                if id_hospital in hospitales:
                    ...
                    print(f"El hospital con ID {id_hospital} no existe. Por favor ingrese un ID válido.")
            else:
    ...
    def agregar_paso_diagnostico(self):
        ...
        sintomas = input("Ingrese los síntomas requeridos (separados por coma) o deje en blanco: ").split(',')
    ...
    def mostrar_secuencia_diagnostico(self):
        sintomas_presentes = input("Ingrese los síntomas presentes (separados por coma): ").split(',')
